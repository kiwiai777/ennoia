...
async function runCmd(fn: () => void | Promise<void>): Promise<RunResult> {
  ...
  try { await fn(); } catch (e) { if (!(e instanceof ProcessExitError)) throw e; }
...
describe('CT-0019: Observation Recap Injection', () => {
  it('默认不带 --with-observation 时不输出 observation recap block', async () => {
    await withTmpHome(async () => {
      await runCmd(async () => cmdContext([]));
      const r = await runCmd(async () => cmdInject([]));
      assert.equal(r.status, 0);
      assert.ok(!r.stdout.includes('[运行时使用摘要（仅供参考）]'));
    });
  });

  it('带 --with-observation 但 observation 为空时不输出 observation recap block', async () => {
    await withTmpHome(async () => {
      const r = await runCmd(async () => cmdInject(['--with-observation']));
      assert.equal(r.status, 0);
      assert.ok(!r.stdout.includes('[运行时使用摘要（仅供参考）]'));
    });
  });

  it('带 --with-observation 且有内容时，generic agent 能够注入 recap block', async () => {
    await withTmpHome(async () => {
  ...
  it('带 --with-observation 且有内容时，claude-code projector 能够注入 recap block', async () => {
    await withTmpHome(async () => {
      await runCmd(async () => cmdContext(['--scope', 'Cortex', '--task-hint', 'planning']));
      const r = await runCmd(async () => cmdInject(['--agent', 'claude-code', '--with-observation']));
      assert.equal(r.status, 0);
      assert.ok(r.stdout.includes('[运行时使用摘要（仅供参考）]'));
      assert.ok(r.stdout.includes('总记录数：1'));
  ...
  it('防回归校验：不会渲染出推断、行动指导等越界措辞', async () => {
    await withTmpHome(async () => {
      await runCmd(async () => cmdContext(['--scope', 'Cortex']));
      const r = await runCmd(async () => cmdInject(['--with-observation']));
  ...
  it('--format json --with-observation 不污染 JSON pack 契约', async () => {
    await withTmpHome(async () => {
      await runCmd(async () => cmdContext(['--scope', 'Cortex']));
      const r = await runCmd(async () => cmdInject(['--format', 'json', '--with-observation']));
      assert.equal(r.status, 0);
  ...
  it('--format json --with-observation --agent claude-code 也不污染 JSON pack 契约', async () => {
    await withTmpHome(async () => {
      await runCmd(async () => cmdContext(['--scope', 'Cortex']));
      const r = await runCmd(async () => cmdInject(['--format', 'json', '--with-observation', '--agent', 'claude-code']));
      assert.equal(r.status, 0);
